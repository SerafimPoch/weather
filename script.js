...
button.addEventListener('click',()=>{let city=input.value
const updateByCity=x=>{let url="https://api.openweathermap.org/data/2.5/forecast?"+"q="+x+"&APPID="+key
sendRequest(url)}
input.value=''
updateByCity(city)})
buttonAdd.addEventListener('click',()=>{let cityAdd=input.value
list.insertAdjacentHTML('beforeend','<li class="list-group-item">'+cityAdd+'</li> ')
input.value=''})
const Faren=c=>Math.round(c*(9/5)-459.67)
const Celc=k=>Math.round(k-273.15)
const sendRequest=url=>{const link=fetch(url).then(data=>data.json()).then(data=>{const main=()=>{let weather={}
...
update(weather)}
buttonF.addEventListener('click',()=>{let weather={}
...
update(weather)})
buttonC.addEventListener('click',()=>{main()})
main()})}
const update=weather=>{data_2.innerHTML=weather.data_2
...
const showPosition=position=>{updateByGeo(position.coords.latitude,position.coords.longitude)}
const updateByGeo=(lat,lon)=>{let url='https://api.openweathermap.org/data/2.5/forecast?'+"lat="+lat+"&lon="+lon+"&APPID="+key
sendRequest(url)}
window.addEventListener('load',()=>{if(navigator.geolocation){navigator.geolocation.getCurrentPosition(showPosition)}else{alert('No internet connection')}})
